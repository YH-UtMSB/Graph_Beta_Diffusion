...
class AbstractDataModule(LightningDataset):
    ...
    def degree_counts(self):
        ...
        for data in self.train_dataloader():
            degs = data.x.flatten().to(torch.int64)
            max_deg = int(degs.max())
            if max_deg >= len(counts):
                counts = torch.cat([counts, torch.zeros(max_deg + 1 - len(counts), dtype=torch.int64)])
            for deg in degs:
                counts[deg] += 1
...
class AbstractDatasetInfos:
    ...
    def compute_input_output_dims(self, datamodule, extra_features, domain_features):
        ...
        ex_dense, node_mask = to_dense(example_batch.x, example_batch.edge_index, example_batch.batch)
