...
def Xdegree_worker(deg_array):
    return np.bincount(deg_array.astype(int))
...
def degree_stats(graph_ref_list, graph_pred_list, is_parallel=True, compute_emd=False):
    ...
    Comment: In version 1.1, degree_stats can be used to evaluate mmd between X-predicted degree and reference graph degrees.
    """
    sample_ref = []
    sample_pred = []

    g0 = next(iter(graph_pred_list))
    if isinstance(g0, nx.Graph):
        ...
        # otherwise g0 is just the node degree vector
        for i in range(len(graph_pred_list)):
            _d = graph_pred_list[i]
            graph_pred_list[i] = _d[np.nonzero(_d)]
    ...
    if is_parallel:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            worker_handle = degree_worker if isinstance(g0, nx.Graph) else Xdegree_worker
            for deg_hist in executor.map(worker_handle, graph_pred_list_remove_empty):
        ...
        for i in range(len(graph_pred_list_remove_empty)):
            if isinstance(g0, nx.Graph):
                ...
                degree_temp = np.bincount(graph_pred_list_remove_empty[i].astype(int))
...
class GenericSamplingMetrics(nn.Module):
    ...
    def forward(self, generated_graphs: list, reference_graphs=None, local_rank=0, test=False):
        ...
        elif isinstance(g0, nx.Graph):
            networkx_graphs = generated_graphs
            for graph in generated_graphs:
                adjacency_matrices.append(nx.to_numpy_array(graph))
       
        elif isinstance(g0, np.ndarray):
            print("CAVEAT: np.ndarray type graph sample detected, only comparing degree stats.")
            self.metrics_list = ['degree']
        
        else:
            raise TypeError(f"generated graph cannot be {type(g0)}")
        ...
        if local_rank == 0:
            print("Samping statistics:")
            print(json.dumps(to_log, indent=4))
        if wandb.run:
